...
def open_interface_1_pay():
    ...
    def activate_function_choose_file():
        sdy_form_izoterma.stex_matrix_to_sdy(filename)
    ...
    button_form_sdy = Button(interface_1_pay, text="Сформировать систему СДУ", command=activate_function_choose_file,
...
def open_interface_2_pay():
    ...
    def choose_parametr_C0():
        ...
        file_param_C0 = filedialog.askopenfilename(initialdir="/", title="Select a File")
        if file_param_C0 != '':
    ...
    def choose_parametr_T0():
        ...
        file_param_T0 = filedialog.askopenfilename(initialdir="/", title="Select a File")
        if file_param_T0 != '':
    ...
    def choose_parametr_E():
        global file_param_E
        file_param_E = filedialog.askopenfilename(initialdir="/", title="Select a File")
        if file_param_E != '':
    ...
    def choose_parametr_k0():
        ...
        file_param_k0 = filedialog.askopenfilename(initialdir="/", title="Select a File")
        if file_param_k0 != '':
    ...
    def run_interface():
        katal_pay.interface_of_graphics(file_param_E, file_param_k0, file_param_T0, file_param_C0)
...
def open_interface_1():
    ...
    # Функция для формирования СДУ
    def activate_function_choose_file():
        sdy_form.stex_matrix_to_sdy(filename)
    ...
    button_select_file.pack(side=TOP)
    # Кнопка Формирования СДУ
    button_form_sdy = Button(interface_1, text="Сформировать систему СДУ", command=activate_function_choose_file,
                             state=tkinter.DISABLED, width=40)
    button_form_sdy.config(font=("Helvetica", 20))
    button_form_sdy.pack(side=TOP)
    # Кнопка перехода на второй интерфейс
...
# Кнопка Формирования СДУ
button_form_sdy = Button(interface_main, text="Каталитическое гидрирование ПАУ", width=40,
